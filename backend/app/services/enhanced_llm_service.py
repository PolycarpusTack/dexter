...
from app.models.ai import ModelStatus as OldModelStatus, OllamaModel
...
class EnhancedLLMService:
    ...
    async def list_models(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_fallback_chain": default_fallback_chain.name
    ...
    async def get_legacy_models(self) -> Dict[str, Any]:
        ...
        try:
            ...
            ollama_status = (
                OldModelStatus.AVAILABLE
                if await self.is_provider_available(ModelProvider.OLLAMA)
                else OldModelStatus.ERROR
            )
    ...
    async def set_user_preferences(
        self, user_id: str, preferences: ModelPreferences
    ) -> Dict[str, Any]:
        ...
        return {"status": "success", "message": f"Model preferences updated for user {user_id}"}
    ...
    async def set_fallback_chain(self, chain_id: str, is_default: bool = False) -> Dict[str, Any]:
    ...
    def _get_provider_for_model(self, model_id: str) -> Optional[ModelProvider]:
        ...
        elif re.match(r"^gpt-\d+", model_id) or model_id in [
            "gpt-4",
            "gpt-3.5-turbo",
            "text-davinci-003",
        ]:
    ...
    def _select_model_for_error(
        self, error_type: Optional[str], error_message: Optional[str], user_id: Optional[str] = None
    ) -> str:
        ...
        if user_id:
            ...
            if preferences:
                ...
                preferences.fallback_models

        # For code-related errors, prefer code-specialized models
        if error_type and any(
            code_term in error_type.lower()
            for code_term in ["syntax", "reference", "type", "null", "undefined", "import"]
        ):
            # Find available code models
            code_model = self.model_registry.get_model_for_task(
                "code", required_capabilities=[ModelCapability.CODE]
            )
            if code_model:
    ...
    async def _execute_with_fallbacks(
        self, action_fn, models: List[str], *args, **kwargs
    ) -> Tuple[str, str, float]:
    ...
    async def get_explanation(
        ...
        try:
            explanation, used_model, processing_time = await self._execute_with_fallbacks(
                get_explanation_from_model, models_to_try
            )
    ...
    def _extract_error_context(self, event_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "exception" in event_data:
            ...
            if isinstance(exception_data, dict) and "values" in exception_data:
                ...
                if len(exception_values) > 0:
                    ...
                    if (
                        "stacktrace" in first_exception
                        and "frames" in first_exception["stacktrace"]
                    ):
        ...
        if (
            not exception_found
            and "entries" in event_data
            and isinstance(event_data["entries"], list)
        ):
    ...
    def _create_prompt(self, event_data: Dict[str, Any]) -> Tuple[str, str]:
        ...
        system_prompt = (
            "You are an expert software engineer specializing in diagnosing and fixing errors. "
        )
        ...
        if context["exception_type"]:
            ...
            if "syntax" in error_type:
                system_prompt += (
                    "You are particularly skilled at identifying and fixing syntax errors in code. "
                )
            elif "reference" in error_type or "undefined" in error_type:
                system_prompt += (
                    "You excel at spotting reference errors and variable scoping issues. "
                )
                ...
                system_prompt += (
                    "You're an expert in authentication, authorization, and permission issues. "
                )

        system_prompt += (
            "Provide clear, concise explanations that help developers understand and fix issues."
        )
        ...
        if context["stack_frames"]:
            ...
            for i, frame in enumerate(context["stack_frames"], 1):
                ...
                if frame["code_context"]:
                    prompt += (
                        f"  Code:\n    {frame['code_context'].replace(chr(10), chr(10) + '    ')}\n"
                    )
        ...
        relevant_tags = [
            ...
            if tag["key"]
            in ["runtime", "environment", "browser", "os", "release", "level", "logger"]
        ]
        if relevant_tags:
            prompt += "\nRELEVANT TAGS:\n"
        ...
        if context["browser"]:
            env_context.append(
                f"Browser: {context['browser']['name']} {context['browser']['version']}"
            )
        if context["os"]:
    ...
    async def set_provider_config(self, provider: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.providers[provider_type] = create_provider(
            provider_type, self.client, provider_settings
        )
    ...
    async def test_provider_connection(
        self, provider: str, request: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if provider_type == ModelProvider.OPENAI:
            ...
            test_config = OpenAIConfig(
                **{**provider_settings.config.dict(), "api_key": SecretStr(api_key)}
            )
            ...
            try:
                ...
                if available:
                    ...
                    try:
                        models = await provider.list_models()
                        available_models = [
                            model.id for model in models if model.status == ModelStatus.AVAILABLE
                        ]

                        results[provider_type.value].update(
                            {"model_count": len(models), "available_models": len(available_models)}
                        )
                    except Exception as e:
                        logger.error(f"Error getting model information for {provider_type}: {e}")
                        results[provider_type.value][
                            "error"
                        ] = f"Error getting model information: {str(e)}"
            except Exception as e:
                logger.exception(f"Error checking availability for provider {provider_type}: {e}")
                results[provider_type.value] = {"available": False, "error": str(e)}
