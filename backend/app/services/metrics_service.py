...
class MetricsService:
    ...
    def _initialize(self):
        ...
        for model_id, model in self.model_registry.models.items():
            ...
            if model_id not in self.token_usage:
                self.token_usage[model_id] = {"input_tokens": 0, "output_tokens": 0}
    ...
    def get_model_metrics(self, model_id: str) -> Dict[str, Any]:
        ...
        # Ensure model exists
        if model_id not in self.historical_metrics:
            return {"model_id": model_id, "status": "No metrics available", "metrics": None}
        ...
        # Response times
        response_times = [
            entry["response_time"] for entry in historical_data if entry["response_time"]
        ]
        ...
        # Token usage
        token_usage = self.token_usage.get(model_id, {"input_tokens": 0, "output_tokens": 0})
    ...
    def get_metrics_by_time_period(
        ...
        response_times = [
            entry["response_time"] for entry in filtered_data if entry["response_time"]
        ]
        avg_response_time = statistics.mean(response_times) if response_times else None
    ...
    def get_time_series_data(
        ...
        for interval_key in sorted(intervals):
            ...
            if metric == "response_time":
                ...
                response_times = [
                    entry["response_time"] for entry in entries if entry["response_time"]
                ]
            ...
            # Add to series
            series_data.append({"timestamp": interval_key, "value": value})
    ...
    def get_provider_metrics(self, provider: ModelProvider) -> Dict[str, Any]:
        ...
        provider_models = [
            model for model in self.model_registry.models.values() if model.provider == provider
        ]
        ...
        for model in provider_models:
            ...
            response_times.extend(
                [entry["response_time"] for entry in hist_data if entry["response_time"]]
            )
        ...
        return {
            ...
            "model_details": [
                {
                    "model_id": m["model_id"],
                    "success_rate": m["historical_metrics"]["performance"]["success_rate"]
                    if m["historical_metrics"]
                    else None,
                    "avg_response_time": m["historical_metrics"]["performance"]["avg_response_time"]
                    if m["historical_metrics"]
                    else None,
                    "requests": m["historical_metrics"]["requests"]["total"]
                    if m["historical_metrics"]
                    else 0,
    ...
    def get_comparison_data(self, model_ids: List[str]) -> Dict[str, Any]:
        ...
        for model_id in model_ids:
            ...
            if not metrics or not metrics["historical_metrics"]:
                comparison.append({"model_id": model_id, "status": "No metrics available"})
                continue
            ...
            comparison_entry = {
                ...
                "provider": model.provider.value if model else None,
                "performance": {
                    "avg_response_time": metrics["historical_metrics"]["performance"][
                        "avg_response_time"
                    ],
                    "success_rate": metrics["historical_metrics"]["performance"]["success_rate"],
                    "percentiles": metrics["historical_metrics"]["performance"].get(
                        "percentiles", {}
                    ),
                },
                "usage": {
        ...
        return {
            ...
            "summary": {
                ...
                "best_response_time": min(
                    [
                        m["performance"]["avg_response_time"]
                        for m in comparison
