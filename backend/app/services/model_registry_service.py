...
class ModelRegistry:
    ...
    def _initialize_defaults(self):
        ...
        self.groups = {
            "code": ModelGroup(
                name="Code Models", models=[], description="Specialized for code analysis"
            ),
            "general": ModelGroup(
                name="General Models", models=[], description="All-purpose models"
            ),
            "small": ModelGroup(
                name="Small Models", models=[], description="Fast, efficient models"
            ),
            "large": ModelGroup(
                name="Large Models", models=[], description="Powerful, high-capacity models"
            ),
        }
    ...
    def update_model_status(
        self, model_id: str, status: ModelStatus, error: Optional[str] = None
    ) -> None:
    ...
    def update_model_metrics(
        self, model_id: str, response_time: Optional[float] = None, success: Optional[bool] = None
    ) -> None:
            ...
            if model.metrics.avg_response_time is None:
                ...
                if model.metrics.usage_count:
                    ...
                    model.metrics.avg_response_time = (
                        1 - weight
                    ) * model.metrics.avg_response_time + weight * response_time
    ...
    def get_model_for_task(
        ...
        if max_size:
            ...
            available_models = [
                m for m in available_models if size_values.get(m.size.value, 0) <= max_size_idx
            ]
