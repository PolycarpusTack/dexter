...
# Only configure root logging if not already configured by the app
if not logging.getLogger().handlers:
    logging.basicConfig(level=settings.LOG_LEVEL.value.upper())
...
class LLMService:
    def __init__(self, client: httpx.AsyncClient):
        ...
        logger.info(
            f"LLM Service initialized for Ollama URL: {self.base_url} using model: {self.model} with timeout: {self.timeout}s"
        )
            ...
            if "models" in models_data and isinstance(models_data["models"], list):
                ...
                for recommended_model in RECOMMENDED_MODELS:
                    if recommended_model not in installed_model_names:
                        models_list.append(
                            OllamaModel(name=recommended_model, status=ModelStatus.UNAVAILABLE)
                        )
    ...
    async def get_explanation(
        ...
        try:
            logger.info(
                f"Sending request to Ollama ({ollama_api_url}) for event {event_id_log} using model: {model_to_use}"
            )
            response = await self.client.post(
                ollama_api_url, json=payload, timeout=float(self.timeout)
            )
            ...
            try:
                ...
                logger.error(
                    f"Ollama returned non-JSON response ({response.status_code}) for event {event_id_log}. Response text: {response.text[:500]}"
                )
                raise HTTPException(
            ...
            logger.info(
                f"Received explanation from Ollama for event {event_id_log} (length: {len(explanation)} chars)"
            )
            ...
            if response_data.get("error"):
                ...
                logger.error(
                    f"Ollama returned error in successful response body for event {event_id_log}: {ollama_error}"
                )
                raise HTTPException(
                    status_code=status.HTTP_502_BAD_GATEWAY,
            ...
            logger.error(
                f"Ollama API request timed out after {self.timeout}s for event {event_id_log}: {ollama_api_url} - {e}"
            )
            raise HTTPException(
            ...
            logger.error(
                f"Ollama API connection error for event {event_id_log}: {ollama_api_url} - {e}"
            )
            ...
            try:
                ...
                ollama_error = ollama_error_body.get(
                ...
                error_detail += (
                    f" - Response: {e.response.text[:200]}"  # Log non-JSON response start
                )

            logger.error(
                f"Ollama API HTTP error for event {event_id_log}: {error_detail} | URL: {e.request.url}"
            )
            ...
            logger.exception(
                f"Unexpected error interacting with LLM service for event {event_id_log}: {e}"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="Unexpected error generating explanation.",
            )
