...
class LLMProviderInterface(ABC):
    ...
    async def generate_response(
        self, model_id: str, prompt: str, parameters: Optional[ModelParameters] = None
    ) -> str:
...
class OllamaProvider(LLMProviderInterface):
    ...
    def _infer_model_size(self, model_name: str, size_bytes: Optional[int] = None) -> ModelSize:
        ...
        for pattern, size in self.size_mapping.items():
            if model_name == pattern or (
                ":" in pattern
                and model_name.startswith(pattern.split(":")[0])
                and pattern.split(":")[1] in model_name
            ):
                return size
    ...
    async def pull_model(self, model_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.model_registry.update_model_status(model_id, ModelStatus.DOWNLOADING)
            ...
            # Update model status in registry
            self.model_registry.update_model_status(model_id, ModelStatus.ERROR, str(e))
    ...
    async def generate_response(
        ...
        try:
            ...
            self.model_registry.update_model_metrics(
                model_id, response_time=response_time, success=True
            )
            ...
            try:
                ...
                logger.error(
                    f"Ollama returned non-JSON response ({response.status_code}). Response text: {response.text[:500]}"
                )

                # Update error metrics
                self.model_registry.update_model_metrics(model_id, success=False)
...
class OpenAIProvider(LLMProviderInterface):
    ...
    def __init__(self, client: httpx.AsyncClient, settings: ProviderSettings):
        ...
        if not self.config.api_key:
            logger.warning(
                "OpenAI API key not set. Provider will be in limited functionality mode."
            )
        ...
        if self.config.organization_slug:
            self.headers["OpenAI-Organization"] = self.config.organization_slug
            ...
            if "data" in data and isinstance(data["data"], list):
                ...
                for model in openai_models:
                    ...
                    if any(
                        ...
                        for prefix in ["gpt-4", "gpt-3.5-turbo", "text-embedding"]
                    ):
            ...
            for model_id in self.config.available_models:
                models.append(
                    Model(
                        ...
                        provider=ModelProvider.OPENAI,
                        status=ModelStatus.UNAVAILABLE,
                        capabilities=self.capability_mapping.get(model_id, [ModelCapability.TEXT]),
                        size=self.size_mapping.get(model_id, ModelSize.LARGE),
        ...
        try:
            ...
            payload = {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are a helpful AI assistant focusing on explaining errors and providing technical assistance.",
                    },
            ...
            # Send request
            response = await self.client.post(
                api_url, headers=self.headers, json=payload, timeout=float(self.timeout)
            )
            response.raise_for_status()
            ...
            raise HTTPException(
                status_code=status.HTTP_502_BAD_GATEWAY, detail=f"OpenAI API error: {error_message}"
            )
...
class AnthropicProvider(LLMProviderInterface):
    ...
    def __init__(self, client: httpx.AsyncClient, settings: ProviderSettings):
        ...
        if not self.config.api_key:
            logger.warning(
                "Anthropic API key not set. Provider will be in limited functionality mode."
            )
    ...
    async def check_availability(self) -> bool:
        ...
        try:
            ...
            response = await self.client.get(api_url, headers=self.headers, timeout=5.0)
            response.raise_for_status()
            return True
        except Exception as e:
...
def create_provider(
    provider_type: ModelProvider, client: httpx.AsyncClient, provider_settings: ProviderSettings
