...
router = APIRouter(
    prefix="/organizations/{organization_slug}",
    tags=["alerts-org"],
    responses={404: {"description": "Not found"}},
)

# Reuse the models from the original alerts router
from .alerts import AlertRuleAction, AlertRuleCondition, AlertRuleFilter, AlertRuleResponse
...
async def create_organization_alert_rule(
    ...
    try:
        ...
        if rule.dataset == "events":
            result = await sentry_client.create_issue_alert_rule(
                organization_slug, project, rule_data
            )
        else:
...
async def update_organization_alert_rule(
    ...
    try:
        ...
        if existing_rule.dataset == "events":
            result = await sentry_client.update_issue_alert_rule(
                organization_slug, rule_id, rule_data
            )
        else:
            result = await sentry_client.update_metric_alert_rule(
                organization_slug, rule_id, rule_data
            )
