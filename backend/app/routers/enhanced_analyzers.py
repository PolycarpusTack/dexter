...
from ..utils.enhanced_deadlock_parser import (
    LOCK_COMPATIBILITY_MATRIX,
    LockMode,
    parse_postgresql_deadlock,
)
...
async def analyze_deadlock_endpoint(
    ...
    try:
        ...
        if isinstance(tags, list):
            ...
            for tag in tags:
                if (
                    tag.get("key") in ["error_code", "db_error_code", "sql_state"]
                    and tag.get("value") == "40P01"
                ):
                    is_deadlock = True
                    break
        else:
            ...
            if (
                tags.get("error_code") == "40P01"
                or tags.get("db_error_code") == "40P01"
                or tags.get("sql_state") == "40P01"
            ):
                is_deadlock = True
        ...
        return {"analysis": response_data, "error": None}

    except Exception as e:
        execution_time = time.time() - start_time
        logger.exception(
            f"Error analyzing deadlock for event {event_id} after {execution_time:.2f}s: {str(e)}"
        )
        ...
        raise HTTPException(
            ...
            detail={"message": f"Failed to analyze deadlock: {str(e)}", "context": error_context},
        )
...
async def lock_compatibility_matrix_endpoint():
    ...
    for mode1 in LockMode:
        ...
        for mode2 in LockMode:
            matrix[mode1.value][mode2.value] = LOCK_COMPATIBILITY_MATRIX.get(mode1, {}).get(
                mode2, False
            )
