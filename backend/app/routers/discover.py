...
from app.services.discover_service import DiscoverService, get_discover_service
...
async def execute_discover_query(
    ...
    try:
        ...
        params = {"field": field_strings, "per_page": query.limit}
        ...
        result = await discover_service.execute_query(
            organization_slug=org_slug, query_params=params
        )
...
async def convert_natural_language_query(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Failed to parse LLM response as valid query")
    except Exception as e:
...
async def get_saved_queries(
    ...
    try:
        ...
        if tags:
            filtered_queries = [
                q for q in filtered_queries if any(tag in q.get("tags", []) for tag in tags)
            ]
...
async def get_query_syntax_help() -> Dict[str, Any]:
    ...
    return {
        ...
        "examples": [
            ...
            {"description": "Find slow transactions", "query": "transaction.duration:>1000"},
            {
                "description": "Find issues affecting specific user",
                "query": "user.email:user@example.com",
            },
            {"description": "Find recent errors", "query": "timestamp:>2024-01-01 level:error"},
        ],
        "functions": {
            "aggregation": [
                "count()",
            ...
            "special": ["failure_rate()", "apdex(threshold)", "impact()"],
        },
        "timeRanges": {
            "relative": [
                "1h - Last hour",
