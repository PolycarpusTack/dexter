...
from app.utils.enhanced_deadlock_parser import model_to_dict, parse_postgresql_deadlock
...
async def list_issue_events(
    ...
    try:
        ...
        events_data = await sentry_client.list_issue_events(
            organization_slug=org_slug, issue_id=issue_id, cursor=cursor, environment=environment
        )
...
async def get_event_details(
    event_id: str,
    project_slug: Optional[str] = Query(None, description="Project ID"),
    sentry_client: SentryApiClient = Depends(get_sentry_client),
):
    logger.info(f"Fetching details for event ID: {event_id}")
    org_slug = get_organization_slug()

    # Get project slug (fallback to settings if not provided)
    project_slug = project_slug or getattr(settings, "project_slug", None)

    try:
        event_data = await sentry_client.get_event_details(
            organization_slug=org_slug, project_slug=project_slug, event_id=event_id
        )
