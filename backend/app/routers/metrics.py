...
from app.services.metrics_service import MetricsService, TimePeriod, get_metrics_service
...
async def get_time_series_data(
    ...
    metric: str = Query(
        "response_time",
        description="Metric to retrieve (response_time, success_rate, request_count, token_usage)",
    ),
...
async def compare_models(
    ...
    try:
        ...
        if not models:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="No model IDs provided"
            )
...
async def get_overall_metrics(metrics_service: MetricsService = Depends(get_metrics_service)):
