...
from ..models.issues import IssueAssignment, IssueStatusUpdate
...
async def list_issues(
    ...
    status: Optional[str] = Query(
        None, description="Filter by status: 'unresolved', 'resolved', 'ignored', or 'all'"
    ),
    ...
    try:
        ...
        # Debug logging
        logger.info(
            f"Issues response being returned: {result.keys() if isinstance(result, dict) else 'not a dict'}"
        )
...
async def update_issue_status(
    ...
    try:
        result = await sentry_client.update_issue_status(
            issue_id=issue_id, status=status_update.status
        )
...
async def assign_issue(
    ...
    try:
        result = await sentry_client.assign_issue(issue_id=issue_id, assignee=assignment.assignee)
