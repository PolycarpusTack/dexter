    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if process_time > 1.0:
                ...
                logger.info(
                    f"Request timing: {request.method} {request.url.path} took {process_time:.4f}s"
                )
...
def setup_middlewares(app: FastAPI, settings: AppSettings) -> None:
    ...
    allowed_headers = list(settings.CORS_ALLOW_HEADERS)
    if "X-CSRF-Token" not in allowed_headers:
        allowed_headers.append("X-CSRF-Token")
    ...
    # Add error handling middleware as an instance so we can access it via app.state
