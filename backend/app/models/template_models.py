...
class PromptTemplate(BaseModel):
    ...
    # Additional metadata
    specific_model: Optional[Dict[str, Any]] = Field(
        None, description="Model-specific configuration", alias="model_specific"
    )
    ...
    def validate_latest_version(cls, v, values):
        ...
        if "versions" in values and values["versions"]:
            ...
            if v not in available_versions:
                raise ValueError(
                    f"Latest version '{v}' not found in versions list: {available_versions}"
                )
        return v
...
class TemplateListResponse(BaseResponse):
    ...
    categories: Dict[TemplateCategory, int] = Field(
        default_factory=dict, description="Count of templates by category"
    )
...
class UpdateTemplateRequest(BaseModel):
    ...
    version_changes: Optional[str] = Field(
...
class RenderTemplateRequest(BaseModel):
    ...
    variables: Dict[str, Any] = Field(
        default_factory=dict, description="Variables to apply to the template"
    )
    version: Optional[str] = Field(
        None, description="Specific version to render (defaults to latest)"
    )
