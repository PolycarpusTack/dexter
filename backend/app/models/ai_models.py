...
class ModelParameters(BaseModel):
    temperature: Optional[float] = Field(
        0.7, description="Controls randomness. Lower values more deterministic."
    )
    top_p: Optional[float] = Field(
        0.9, description="Controls diversity. Lower values more focused."
    )
    max_tokens: Optional[int] = Field(2048, description="Maximum number of tokens to generate.")
    frequency_penalty: Optional[float] = Field(
        0, description="Reduces repetition of token sequences."
    )
...
class ModelPreferences(BaseModel):
    ...
    parameters: Optional[Dict[str, ModelParameters]] = Field(
        default_factory=dict, description="Model-specific parameters"
    )
    default_parameters: Optional[ModelParameters] = Field(
        default=ModelParameters(), description="Default parameters for all models"
    )
...
class Model(BaseModel):
    ...
    parameters: ModelParameters = Field(
        default_factory=ModelParameters, description="Default parameters for this model"
    )
    metrics: Optional[ModelMetrics] = Field(
        default_factory=ModelMetrics, description="Usage metrics for this model"
    )
...
class ModelsResponse(BaseModel):
    ...
    groups: Optional[List[ModelGroup]] = Field(
        default_factory=list, description="Grouped models by category"
    )
    fallback_chains: Optional[List[FallbackChain]] = Field(
        default_factory=list, description="Available fallback chains"
    )
...
class ModelRequest(BaseModel):
    ...
    fallback_chain_id: Optional[str] = Field(
        None, description="ID of fallback chain to use if specified"
    )
