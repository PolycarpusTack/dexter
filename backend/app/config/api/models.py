...
class ApiEndpoint(BaseModel):
    ...
    rate_limited: bool = Field(
        default=True, description="Whether endpoint is subject to rate limiting"
    )
    ...
    response_model: Optional[str] = Field(
        default=None, description="Pydantic model name for response validation"
    )
...
class ApiCategory(BaseModel):
    ...
    base_path: Optional[str] = Field(
        default=None, description="Base path prefix for all endpoints in category"
    )
    endpoints: Dict[str, ApiEndpoint] = Field(
        default_factory=dict, description="Mapping of endpoint names to configurations"
    )
...
class ApiPathConfig(BaseModel):
    ...
    categories: Dict[str, ApiCategory] = Field(
        default_factory=dict, description="Mapping of category names to configurations"
    )
