...
class OpenAIConfig(BaseModel):
    ...
    organization_slug: Optional[str] = Field(
        None, description="OpenAI organization ID for usage tracking."
    )
    ...
    # Rate limiting
    requests_per_minute: int = Field(60, description="Maximum number of requests per minute.", ge=1)

    # Timeout settings
    timeout: float = Field(30.0, description="Request timeout in seconds.", ge=1.0)
    ...
    available_models: Set[str] = Field(
        {"gpt-4o", "gpt-4-turbo", "gpt-4", "gpt-3.5-turbo"}, description="Set of available models."
    )
    ...
    # Usage tracking
    track_usage: bool = Field(True, description="Whether to track API usage.")

    # Retry settings
    max_retries: int = Field(3, description="Maximum number of retries for failed requests.", ge=0)
    retry_delay: float = Field(1.0, description="Initial delay between retries in seconds.", ge=0.1)
    retry_multiplier: float = Field(
        2.0, description="Multiplier for increasing retry delay (exponential backoff).", ge=1.0
    )

    # Advanced settings
    streaming: bool = Field(False, description="Whether to use streaming responses by default.")
    use_azure: bool = Field(
        False, description="Whether to use Azure OpenAI instead of the standard OpenAI API."
    )
