...
class QueryFingerprint(BaseModel):
    ...
    def from_query(cls, query: str) -> "QueryFingerprint":
        ...
        if not query:
            return cls(original_query="", normalized_query="", hash="")
...
class Transaction(BaseModel):
    ...
    locks_waiting: List[int] = Field(
        default_factory=list
    )  # IDs of locks this transaction is waiting for
...
def redact_pii_from_query(query: str) -> str:
    ...
    # Redact UUIDs
    query = re.sub(
        r"\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b", "[UUID]", query
    )
...
def _extract_transactions(raw_info: Dict[str, Any], message: str) -> List[Transaction]:
    ...
    for pid in all_pids:
        ...
        query_pattern = compile_regex(
            rf"Process {pid}:.*?statement: (.*?)(?=Process \d+:|$)", re.DOTALL
        )
        ...
        app_pattern = compile_regex(rf"Process {pid}: application_name: ([^\n]+)")
        app_match = app_pattern.search(message)
        app_name = app_match.group(1).strip() if app_match else None

        # Extract username if available
        user_pattern = compile_regex(rf"Process {pid}: user=([^\s,]+)")
...
def _extract_locks(raw_info: Dict[str, Any], message: str) -> List[LockInfo]:
    ...
    lock_pattern = compile_regex(
        r"((?:Share|Update|Exclusive)(?:Lock)) on ([^\s]+)(?: ([^\s]+))? (granted|waiting)"
    )
    for match in lock_pattern.finditer(message):
        ...
        context_pattern = compile_regex(
            rf"Process (\d+).*?{lock_mode}.*?{lock_type}.*?{status}", re.DOTALL
        )
            ...
            for other_tx in transactions:
                ...
                for held_lock_id in other_tx.locks_held:
                    ...
                    if (
                        ...
                        and waiting_lock.relation == held_lock.relation
                        and not waiting_lock.is_compatible_with(held_lock)
                    ):
...
def _find_deadlock_cycles(
    graph: nx.DiGraph, transactions: List[Transaction], locks: List[LockInfo]
) -> List[DeadlockCycle]:
    ...
    try:
        ...
        for cycle in nx.simple_cycles(graph):
            if len(cycle) > 1:  # Ignore self-cycles
                        ...
                        if 0 <= waiting_lock_id < len(locks):
                            ...
                            if waiting_lock.relation:
                                ...
                                if isinstance(waiting_lock.lock_mode, LockMode):
                                    if waiting_lock.lock_mode in [
                                        LockMode.EXCLUSIVE,
                                        LockMode.ACCESS_EXCLUSIVE,
                                    ]:
                                        severity += 5
                                    elif waiting_lock.lock_mode in [
                                        LockMode.SHARE_ROW_EXCLUSIVE,
                                        LockMode.SHARE,
                                    ]:
                                        severity += 3
                ...
                cycles.append(
                    DeadlockCycle(processes=cycle, relations=list(relations), severity=severity)
                )
    except Exception as e:
...
def _generate_recommendation(deadlock_info: DeadlockInfo) -> str:
    ...
    if all_tables:
        ...
        has_exclusive_locks = any(
            (
                "EXCLUSIVE" in str(lock.lock_mode).upper()
                and "ROW" not in str(lock.lock_mode).upper()
            )
            for lock in locks
        )
...
The deadlock occurred because multiple transactions were trying to acquire locks ({lock_modes_str}) on the same tables but in different orders,
    creating a circular waiting pattern.
...
The deadlock involved SELECT FOR UPDATE statements,
    which acquire exclusive row locks that can easily conflict with other transactions.
...
The deadlock involved exclusive locks,
    which block most other lock types and frequently cause deadlocks when multiple transactions attempt to acquire them in different orders.
...
def _prepare_visualization_data(deadlock_info: DeadlockInfo, graph: nx.DiGraph) -> Dict[str, Any]:
        ...
        nodes.append(
            {
                ...
                "locks_held": [
                    locks[lock_id].get_formatted_description()
                    for lock_id in tx.locks_held
                    if lock_id < len(locks)
                ],
                "locks_waiting": [
                    locks[lock_id].get_formatted_description()
                    for lock_id in tx.locks_waiting
                    if lock_id < len(locks)
                ],
    ...
    for u, v, data in graph.edges(data=True):
        ...
        waiting_lock = (
            locks[waiting_lock_id]
            if waiting_lock_id is not None and waiting_lock_id < len(locks)
            else None
        )
        blocking_lock = (
            locks[blocking_lock_id]
            if blocking_lock_id is not None and blocking_lock_id < len(locks)
            else None
        )
    ...
    for mode1 in LockMode:
        ...
        for mode2 in LockMode:
            lock_compatibility[mode1.value][mode2.value] = LOCK_COMPATIBILITY_MATRIX.get(
                mode1, {}
            ).get(mode2, False)
    ...
    return {
        ...
        "edges": edges,
        "cycles": [
            {"processes": cycle.processes, "relations": cycle.relations, "severity": cycle.severity}
            for cycle in cycles
        ],
