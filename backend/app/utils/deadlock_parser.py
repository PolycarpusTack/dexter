...
def _extract_locks(raw_info: Dict[str, Any], message: str) -> List[LockInfo]:
    ...
    for match in re.finditer(lock_pattern, message):
        ...
        context_pattern = rf"Process (\d+).*?{lock_mode}.*?{lock_type}.*?{status}"
...
def _build_transaction_graph(transactions: List[Transaction], locks: List[LockInfo]) -> nx.DiGraph:
    ...
    for tx in transactions:
        ...
        waiting_locks = [
            lock for lock in locks if lock.process_id == tx.process_id and not lock.granted
        ]
        ...
        for waiting_lock in waiting_locks:
            ...
            for other_tx in transactions:
                ...
                granted_locks = [
                    lock
                    for lock in locks
                    if lock.process_id == other_tx.process_id and lock.granted
                ]
                ...
                for granted_lock in granted_locks:
                    if granted_lock.relation == waiting_lock.relation:
                        ...
                        tx.locks_waiting.append(
                            f"{waiting_lock.lock_mode} on {waiting_lock.relation}"
                        )
                        other_tx.locks_held.append(
                            f"{granted_lock.lock_mode} on {granted_lock.relation}"
                        )
    ...
    try:
        ...
        for cycle in nx.simple_cycles(graph):
            if len(cycle) > 1:  # Ignore self-cycles
                ...
                cycles.append(DeadlockCycle(processes=cycle, relations=list(relations)))
    except Exception as e:
...
def _generate_recommendation(
    transactions: List[Transaction], locks: List[LockInfo], cycles: List[DeadlockCycle]
) -> str:
...
The deadlock occurred because multiple transactions were trying to acquire locks on the same tables but in different orders,
...
def _prepare_visualization_data(
    ...
    return {
        ...
        "edges": edges,
        "cycles": [
            {"processes": cycle.processes, "relations": cycle.relations} for cycle in cycles
        ],
    }
